    ...
    const fetchCategories = async () => {
      ...
      try {
        const res = await axios.get(
          `${API_BASE_URL}/api/admin/manage-app/categories`,
          { timeout: 15000 },
        );
        if (!mounted) return;
        if (res?.data?.success && Array.isArray(res.data.categories)) {
          const names = res.data.categories
            .map((c) => (c?.name ? String(c.name).trim() : null))
            .filter(Boolean);
          const unique = Array.from(new Set(["All Products", ...names]));
          setCategories(unique);
        } else {
          console.warn(
            "Categories API returned no categories, keeping fallback",
          );
        }
      } catch (err) {
      ...
      >
        ...
        {fetchError && (
          <View style={{ padding: moderateScale(8) }}>
            <Text
              allowFontScaling={false}
              style={{ color: "#c00", fontSize: normalizeFont(12) }}
            >
              Could not load latest categories — showing cached options.
            </Text>
          </View>
        )}
        ...
        {categories.map((category) => {
          ...
          const isExpanded =
            expandedCategory && sameCategory(expandedCategory, category);

          return (
            <View
              key={String(category)}
              style={dropdownStyles.categoryContainer}
            >
              <View style={dropdownStyles.categoryRow}>
                ...
                >
                  ...
                  <Text
                    allowFontScaling={false}
                    style={dropdownStyles.categoryText}
                  >
                    {category}
                  </Text>
                </TouchableOpacity>
                ...
                {category !== "All Products" && categoryProducts.length > 0 && (
                  ...
                  >
                    <Text
                      allowFontScaling={false}
                      style={dropdownStyles.expandArrow}
                    >
                      {isExpanded ? "▲" : "▼"}
                    </Text>
                  </TouchableOpacity>
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          if (typeof onProductSelect === "function")
                            onProductSelect(product._id, category);
                        }}
                      >
                        <Text
                          allowFontScaling={false}
                        ...
                        {isProductSelected(product._id) && (
                          <Text
                            allowFontScaling={false}
                            style={dropdownStyles.productCheckmark}
                          >
                            ✓
                          </Text>
                        )}
      ...
      >
        ...
        >
          ...
          <View style={createModalStyles.header}>
            ...
            <Text
              allowFontScaling={false}
              style={createModalStyles.headerTitle}
            >
              {title}
            </Text>
          ...
          >
            ...
            <View style={createModalStyles.inputGroup}>
              <TextInput
                ...
                allowFontScaling={false}
              />
              <Text allowFontScaling={false} style={createModalStyles.starIcon}>
                ✨
              </Text>
            </View>
              ...
              <View style={createModalStyles.halfInput}>
                ...
                <View style={createModalStyles.discountInputGroup}>
                  ...
                  <View style={createModalStyles.discountDropdown}>
                    <Text
                      allowFontScaling={false}
                      style={createModalStyles.discountText}
                    >
                      %
                    </Text>
                  </View>
            ...
            <View style={createModalStyles.row}>
              <View style={createModalStyles.halfInput}>
                ...
                >
                  <Text
                    allowFontScaling={false}
                    style={createModalStyles.dateText}
                  >
            ...
            <View style={[createModalStyles.inputGroup, { zIndex: 100 }]}>
              <View style={createModalStyles.categoryInputWrapper}>
                ...
                >
                  ...
                  <Text
                    allowFontScaling={false}
                    style={createModalStyles.dropdownArrow}
                  >
            ...
            {productsLoading && (
              <Text
                allowFontScaling={false}
                style={createModalStyles.loadingText}
              >
                Loading products...
              </Text>
            )}
          ...
          >
            {loading ? (
              ...
              <Text
                allowFontScaling={false}
                style={createModalStyles.createButtonText}
              >
                {submitButtonText}
              </Text>
            )}
...
const MyCoupons = () => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        setOriginalCoupons((prev) =>
          prev.map((coupon) =>
            String(coupon.id) === String(updatedCoupon.id)
              ? updatedCoupon
              : coupon,
          ),
        );
  ...
  const handleDeleteCoupon = async (couponOrId) => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "URL DELETE failed, retrying with body if applicable:",
          errUrlDelete?.response?.data ?? errUrlDelete.message,
        );
        try {
          ...
          console.error(
            "Both delete-by-URL and delete-by-body failed:",
            errBodyDelete?.response?.data ?? errBodyDelete.message,
          );
          throw errBodyDelete;
        }
      }
    ...
    if (pageX != null && pageY != null) {
      ...
      if (top + modalEstimatedHeight > SCREEN_HEIGHT - 8) {
        ...
        top =
          aboveTop > 8
            ? aboveTop
            : Math.max(SCREEN_HEIGHT - modalEstimatedHeight - 8, 8);
      }
    }
  ...
  const handleDelete = () => {
    const idToDelete =
      selectedCoupon?.id ?? selectedCoupon?._id ?? selectedCoupon?.couponId;
    if (!idToDelete) {
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Delete",
          style: "destructive",
          onPress: () => handleDeleteCoupon(idToDelete),
        },
      ],
    );
  };
        ...
        <View style={styles.actionRow}>
          ...
          <View style={styles.filterDropdownWrapper}>
            ...
            >
              ...
              <Text allowFontScaling={false} style={styles.dropdownArrow}>
                ▼
              </Text>
            </TouchableOpacity>
                ...
                <View key={String(key)} style={styles.couponCard}>
                  <View style={styles.cardContent}>
                    <View style={styles.textRow}>
                      <Text allowFontScaling={false} style={styles.couponLabel}>
                        Code
                      </Text>
                      <Text
                        allowFontScaling={false}
                  ...
                  <TouchableOpacity
                    ...
                    onPressIn={(e) =>
                      handleThreeDotsPress(coupon, index, e.nativeEvent)
                    }
                    activeOpacity={0.7}
                  >
                    <Text allowFontScaling={false} style={styles.threeDotsText}>
                      ⋮
                    </Text>
                  </TouchableOpacity>
                ...
                >
                  ...
                  >
                    {deleteLoading && selectedCoupon ? (
                      ...
                      <Text
                        allowFontScaling={false}
                        style={[styles.modalOptionText, styles.deleteText]}
                      >
                        Delete
                      </Text>
                    )}
