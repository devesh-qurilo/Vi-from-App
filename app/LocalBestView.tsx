...
import LocalVendorr from "../components/common/LocalVendor";
...
const ProductCard = React.memo(
  ({
    ...
    // modal state for editing exact quantity
            ...
            <View style={cardStyles.buttonContainer}>
              {!inCart ? (
                ...
                >
                  <Text
                    allowFontScaling={false}
                    style={cardStyles.addToCartText}
                  >
    ...
    async (product, change) => {
      ...
      if (newQuantity < 1) {
        ...
        try {
          ...
          await fetchCart();
          if (!err.response)
            Alert.alert("Network Error", "Failed to update cart");
        }
      } else {
        setCartItems((prev) => ({
          ...prev,
  ...
  const openProductDetails = useCallback(
    (product) => {
      try {
        ...
        Alert.alert(
          "Navigation Error",
          "Could not open product details. See console.",
        );
      }
    },
    [navigation],
      ...
      {loading ? (
        ...
        <FlatList
          ...
          keyExtractor={(item, index) =>
            item._id || item.id || String(item?.name) || String(index)
          }
          numColumns={2}
