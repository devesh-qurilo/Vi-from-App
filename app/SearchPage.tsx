...
const SearchPage = () => {
    ...
    PanResponder.create({
      ...
      onPanResponderMove: (e, { dx }) => {
        const newValue = Math.max(
          50,
          Math.min(
            tempFilters.priceMax - 100,
            tempFilters.priceMin + Math.round(dx / 2),
          ),
        );
    ...
    PanResponder.create({
      ...
      onPanResponderMove: (e, { dx }) => {
        const newValue = Math.min(
          3000,
          Math.max(
            tempFilters.priceMin + 100,
            tempFilters.priceMax + Math.round(dx / 2),
          ),
        );
    ...
    PanResponder.create({
      ...
      onPanResponderMove: (e, { dx }) => {
        const newValue = Math.max(
          0,
          Math.min(
            tempFilters.distanceMax - 5,
            tempFilters.distanceMin + Math.round(dx / 3),
          ),
        );
    ...
    PanResponder.create({
      ...
      onPanResponderMove: (e, { dx }) => {
        const newValue = Math.min(
          100,
          Math.max(
            tempFilters.distanceMin + 5,
            tempFilters.distanceMax + Math.round(dx / 3),
          ),
        );
  ...
  const applyFiltersToProducts = () => {
    ...
    if (sortBy && sortBy !== "relevance") {
      if (sortBy === "Price - low to high") {
        sorted.sort(
          (a, b) =>
            Number(a.price ?? a.mrp ?? 0) - Number(b.price ?? b.mrp ?? 0),
        );
      } else if (sortBy === "Price - high to low") {
        sorted.sort(
          (a, b) =>
            Number(b.price ?? b.mrp ?? 0) - Number(a.price ?? a.mrp ?? 0),
        );
  ...
  const handleUpdateQuantity = async (product, change) => {
    try {
      ...
      if (typeof change === "number") {
        newQuantity = Number((currentItem.quantity + change).toFixed(1));
      } else {
      ...
      if (newQuantity < 0.1) {
        ...
        const response = await axios.delete(
          `${API_BASE_URL}/api/buyer/cart/${currentItem.cartItemId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
        ...
        setCartItems((prev) => ({
          ...
          [productId]: {
            ...currentItem,
            quantity: Number(newQuantity.toFixed(1)),
          },
        }));
  ...
  const applyQuantityChange = async () => {
    ...
    let parsed = parseFloat(editQuantity.replace(/[^0-9.]/g, ""));
    const decimalCount = editQuantity.split(".").length - 1;
    if (decimalCount > 1) {
      Alert.alert(
        "Invalid Quantity",
        "Please enter a valid quantity with only one decimal point",
      );
      return;
    }
    ...
    if (newQty < 0.1) {
      ...
      await handleUpdateQuantity(selectedProduct, String(newQty));
    }
  ...
  const incrementEditQuantity = () => {
    const v = parseFloat(editQuantity || "0") || 0;
    setEditQuantity(String((v + 1).toFixed(1)));
  };

  const decrementEditQuantity = () => {
    const v = parseFloat(editQuantity || "0") || 0;
    setEditQuantity(String(Math.max(0.1, (v - 1).toFixed(1))));
  };
  ...
  const ProductCard = ({ product }) => {
    ...
    const imageUri =
      product.images?.[0] || product.image || "https://via.placeholder.com/120";
    ...
    const status =
      product.status || (product.quantity === 0 ? "Out of Stock" : "In Stock");
      ...
      >
        ...
        >
          <View style={styles.quantityModalContent}>
            <Text allowFontScaling={false} style={styles.quantityModalTitle}>
              Edit Quantity
            </Text>
          ...
          >
                              ...
                              <Text
                                ...
                                style={[
                                  ...
                                  tempFilters.sortBy === option &&
                                    styles.filterOptionText2Active,
                                ]}
                              >
                                {option}
                              </Text>
                          ...
                          <View style={styles.sliderContainer}>
                            <View style={styles.sliderLabelRow}>
                              <Text
                                ...
                                style={styles.sliderLabel}
                              >
                                {tempFilters.distanceMin}km
                              </Text>
                              <Text
                                allowFontScaling={false}
                                style={styles.sliderLabel}
                              >
                                {tempFilters.distanceMax}km
                              </Text>
                            </View>
                            <View style={styles.sliderBar}>
                              <View
            ...
            <View style={styles.filterFooter}>
              ...
              >
                <Text
                  allowFontScaling={false}
                  style={[styles.applyButtonText, { color: "#333" }]}
                >
                  Clear Filters
                </Text>
              </TouchableOpacity>
