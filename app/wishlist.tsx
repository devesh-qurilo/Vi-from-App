        ...
        <View style={cardStyles.cardContent}>
          ...
          <View style={cardStyles.priceContainer}>
            ...
            {weight ? (
              <Text
                allowFontScaling={false}
                style={{ fontSize: normalizeFont(12), fontWeight: "bold" }}
              >
                /{weight}
              </Text>
            ) : null}
...
const MyWishlist = () => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        const mapped = items.map((w, idx) => {
          const product =
            w.product || w.productDetails || w.productData || null;
          if (product) {
            return {
              ...product,
              id: product._id || product.id || product.productId || `wl-${idx}`,
              productId:
                product._id || product.id || product.productId || `wl-${idx}`,
            };
          }
      ...
      try {
        ...
        if (res.data?.success) {
          ...
          const favSet = new Set(
            items.map((i) => i.productId || i._id || i.id),
          );
          setFavorites(favSet);
        }
      } catch (e) {
  ...
  const removeFromWishlist = async (product) => {
    try {
      ...
      if (res.data?.success) {
        ...
        Alert.alert(
          "Wishlist",
          res.data?.message || "Could not remove from wishlist",
        );
      }
    } catch (err) {
  ...
  const openProductDetails = (product) => {
    try {
      ...
      navigation.push
        ? navigation.push("ViewProduct", { productId, product })
  ...
  useEffect(() => {
    const getAllCategory = async () => {
      try {
        ...
        const onlyNames =
          catRes.data?.categories?.map((item) => item.name) || [];
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.filterWrapper}>
          ...
          >
            ...
            <Image
              source={require("../assets/via-farm-img/icons/expandArrow.png")}
          ...
          <Animated.View
            style={[
              styles.dropdown,
              { height: dropdownHeight, borderWidth: borderWidth },
            ]}
          >
            <ScrollView>
              {options.map((opt) => (
      ...
      {loading ? (
        ...
        selectedOption === "All" ? (
          ...
          <View style={styles.centerContainer}>
            <Text allowFontScaling={false} style={styles.emptyText}>
              No items found for {selectedOption}
            </Text>
            <TouchableOpacity
              style={styles.retryButton}
        ...
        <FlatList
          ...
          keyExtractor={(item, idx) =>
            item._id || item.id || item.productId || String(idx)
          }
          numColumns={2}
