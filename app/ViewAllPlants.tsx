      ...
      >
        ...
        <View style={cardStyles.cardContent}>
          ...
          <View style={cardStyles.priceContainer}>
            ...
            {item?.weightPerPiece ? (
              <Text style={cardStyles.weightText}>{item.weightPerPiece}</Text>
            ) : null}
          </View>
  ...
  const fetchPlants = async () => {
    try {
      ...
      if (res.data && res.data.success) {
        ...
        setPlants(Array.isArray(arr) ? arr : []);
      } else {
        setPlants([]);
        setError("No plants found in your area");
      }
    } catch (err) {
    ...
    try {
      ...
      if (r.data?.success) {
        setCartItems((prev) => ({
          ...
          [productId]: {
            quantity: 1,
            cartItemId: r.data.data?._id || productId,
          },
        }));
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
      ...
      {loading && (
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Fetching plants...</Text>
        </View>
      )}
        ...
        <>
          {filteredPlants.length === 0 ? (
            <View style={{ padding: 20, alignItems: "center" }}>
              <Text style={{ color: "#444" }}>No plants match “{query}”</Text>
            </View>
          ) : (
            <FlatList
              data={filteredPlants}
