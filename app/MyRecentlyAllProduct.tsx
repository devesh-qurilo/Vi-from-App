...
const AllRecently = () => {
  ...
  useEffect(() => {
    const getAllCategory = async () => {
      try {
        ...
        const onlyNames = (catRes.data?.categories || [])
          .map((item) => item.name)
          .filter(Boolean);
  ...
  const submitModal = (updatedProduct) => {
    ...
    setFilteredData((prev) =>
      prev.map((i) => (i.id === updatedProduct.id ? updatedProduct : i)),
    );
    closeModal();
  };
  ...
  const openStockDropdown = (productId) => {
    ...
    if (ref && ref.measureInWindow) {
      ...
      const pos = clampPos(
        width / 2 - moderateScale(80),
        height / 2 - moderateScale(40),
      );
      setStockDropdownPosition({ x: pos.x, y: pos.y });
      setCurrentProductId(productId);
      setIsStockDropdownOpen(true);
  ...
  const openCategoryDropdown = () => {
    if (
      ...
      categoryButtonRef.current.measureInWindow
    ) {
          ...
          onPress: async () => {
            try {
              ...
              setListingsData((prev) =>
                prev.filter((item) => item.id !== currentProductId),
              );
              setFilteredData((prev) =>
                prev.filter((i) => i.id !== currentProductId),
              );
              ...
              try {
                ...
                if (!ok) {
                  console.log(
                    "Delete not acknowledged by server, refetching",
                    response?.data,
                  );
                  await fetchProducts();
                } else {
                ...
                console.log(
                  "Delete request failed, refetching:",
                  err?.message || err,
                );
                await fetchProducts();
              }
  ...
  const handleStockChange = async (newStatus) => {
    ...
    try {
      ...
      setFilteredData((prev) =>
        prev.map((i) =>
          i.id === currentProductId ? { ...i, status: newStatus } : i,
        ),
      );
      ...
      const tryEndpoints = [
        {
          method: "patch",
          url: `${API_BASE}/api/vendor/products/${currentProductId}/status`,
          data: { status: newStatus },
        },
        {
          method: "patch",
          url: `${API_BASE}/api/vendor/products/${currentProductId}`,
          data: { status: newStatus },
        },
        {
          method: "put",
      ...
      for (let ep of tryEndpoints) {
        try {
          ...
          if (
            resp &&
            (resp.status === 200 || resp.status === 204 || resp.data?.success)
          ) {
            succeeded = true;
      ...
      if (!succeeded) {
        console.log(
          "All stock endpoints failed, resyncing from server for product:",
          currentProductId,
        );
        await fetchProducts();
      }
  ...
  const handleCardPress = (item) => {
    try {
      if (navigation?.push) {
        navigation.push("VendorViewProduct", {
          productId: item.id,
          product: item._raw ?? item,
        });
      } else if (navigation?.navigate) {
        navigation.navigate("VendorViewProduct", {
          productId: item.id,
          product: item._raw ?? item,
        });
      } else {
        ...
        <View style={styles.cardInner}>
            ...
            {item.image ? (
              ...
              <View style={styles.noImage}>
                <Text
                  ...
                  style={{ color: "#999", fontSize: normalizeFont(12) }}
                >
                  No image
                </Text>
              </View>
          ...
          <View style={styles.rightContent}>
            <View style={styles.titleRow}>
              ...
              >
                <Image
                  source={require("../assets/via-farm-img/icons/threeDot.png")}
                />
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.infoRow}>
              ...
              <Text allowFontScaling={false} style={styles.infoSeparator}>
                :
              </Text>
            ...
            <View style={styles.uploadRow}>
              <Text allowFontScaling={false} style={styles.uploadLabel}>
                Uploaded on
              </Text>
              <Text allowFontScaling={false} style={styles.uploadValue}>
                {item.uploadedOn}
              </Text>
            </View>
            ...
            <View style={styles.stockRow}>
              ...
              >
                <View
                  style={[styles.stockDot, { backgroundColor: circleColor }]}
                />
                <Text allowFontScaling={false} style={styles.stockText}>
                  {item.status}
                </Text>
                <Image
  ...
  const renderHeader = () => (
    <View style={styles.header}>
      ...
      >
        <Text allowFontScaling={false} style={styles.categoryDropdownText}>
          {selectedCategory}
        </Text>
            ...
            <TouchableWithoutFeedback>
              ...
              >
                ...
                >
                  ...
                  />
                  <Text allowFontScaling={false} style={styles.stockOptionText}>
                    In Stock
                  </Text>
                </TouchableOpacity>
                  ...
                  {(categories.length ? categories : ["All"]).map(
                    (category, index) => (
                      <View key={category + index}>
                        ...
                        {index <
                          (categories.length ? categories.length : 1) - 1 && (
            ...
            <TouchableWithoutFeedback>
              ...
              >
                ...
                >
                  <Text
                    allowFontScaling={false}
                    style={[styles.actionOptionText, styles.deleteText]}
                  >
                    Delete
                  </Text>
                </TouchableOpacity>
