...
const VendorsDetails = () => {
  ...
  const cardWidth = Math.max(
    120,
    Math.floor((window.width - horizontalPadding - columnGap) / CARD_COLUMNS),
  );
  ...
  const fetchVendor = async () => {
    try {
      ...
      const resp = await axios.get(
        `${API}/api/buyer/vendor/${vendorId}${COORDS}`,
        {
      ...
      const reviewsList =
        data?.data?.reviews?.list ??
        data?.data?.reviews ??
        data?.reviews?.list ??
        data?.reviews ??
        [];
      const farmImagesList =
        vendorData?.farmImages ??
        data?.data?.farmImages ??
        data?.farmImages ??
        [];
      const listedProducts =
        data?.data?.listedProducts ?? data?.listedProducts ?? [];
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        const favIds = new Set(
          wishlistItems.map((it: any) => it.productId || it._id || it.id),
        );
        setFavorites(favIds);
      }
    } catch (err) {
    ...
    (async () => {
      try {
        ...
        const res = await axios.delete(
          `${API}/api/buyer/wishlist/${productId}`,
          { headers: { Authorization: `Bearer ${token}` } },
        );
  ...
  const handleAddToCart = (product: any) => {
    ...
    setCartItems((prev) => ({
      ...prev,
      [productId]: { quantity: 1, cartItemId: productId },
    }));
    ...
    if (newQty < 1) {
      ...
      (async () => {
        try {
          ...
          const res = await axios.delete(
            `${API}/api/buyer/cart/${current.cartItemId}`,
            { headers: { Authorization: `Bearer ${token}` } },
          );
          if (!res.data?.success) {
            setCartItems(prevCart);
    ...
    (async () => {
      try {
        ...
        const res = await axios.put(
          `${API}/api/buyer/cart/${current.cartItemId}/quantity`,
          { quantity: newQty },
          { headers: { Authorization: `Bearer ${token}` } },
        );
        if (!res.data?.success) {
          ...
          try {
            await axios.put(
              `${API}/api/buyer/cart/update`,
              { cartItemId: current.cartItemId, quantity: newQty },
              { headers: { Authorization: `Bearer ${token}` } },
            );
          } catch (inner) {
  ...
  if (error || !vendor)
    return (
      <View style={styles.containerRoot}>
        <View style={styles.center}>
          <Text allowFontScaling={false} style={styles.errorHeader}>
            Error
          </Text>
    ...
    >
      ...
      <View
        style={{
          ...
          marginTop: allReviewImages.length > 0 ? 0 : 10,
        }}
      >
        {farmImage.length > 0 && (
          <FlatList
            data={farmImage}
            renderItem={({ item }) => (
              <FarmImage item={item} onPress={openImageViewer} />
            )}
        ...
        >
          ...
          >
            <Text allowFontScaling={false} style={styles.sectionHeader}>
              ...
              <Text
                allowFontScaling={false}
                style={{ fontSize: normalizeFont(12), fontWeight: "400" }}
              >
            ...
            <TouchableOpacity
              onPress={() =>
                (navigation as any).navigate("SeeAllReview", {
                  vendor: v,
                  reviews,
                })
              }
      ...
      >
        {reviews.length > 0 ? (
          <FlatList
            ...
            horizontal
            keyExtractor={(item: any, idx: number) =>
              item?._id ? item._id.toString() : String(idx)
            }
          ...
          >
            <Image
              ...
              source={require("../assets/via-farm-img/icons/empty.png")}
            />
            <View
              style={{
                flexDirection: "row",
      ...
      <View style={[styles.productsHeaderContainer]}>
        <Text
          allowFontScaling={false}
          style={[styles.sectionHeader, { marginLeft: 0, marginVertical: 0 }]}
        >
          Listing Product
        </Text>
            ...
            style={[
              styles.dropdown,
              {
                height: animation.interpolate({
                  inputRange: [0, 1],
                  outputRange: [0, moderateScale(240)],
                }),
...
//         <View style={styles.row}>
//           <Image style={{width:scale(20), height:scale(28)}} source={require('../assets/via-farm-img/icons/mainLocationIcon.png')} />
...
//   ratingText: { fontWeight: '700' },
//   row: { flexDirection: 'row', alignItems: 'center', gap:3, marginTop: moderateScale(8) },
