...
const ReviewOrder = () => {
  ...
  const normalizeAddress = (addr) => {
    ...
    return {
      ...
      formattedAddress:
        `${addr.houseNumber || ""} ${addr.locality || ""} ${addr.city || ""} ${addr.state || ""}`
          .replace(/\s+/g, " ")
          .trim(),
    };
  };
  ...
  const fetchCartProducts = async () => {
    try {
      ...
      if (response.data && response.data.success) {
        ...
        const applied =
          response.data.data?.appliedCoupon ??
        ...
        if (applied) {
          if (AUTO_CLEAR_SERVER_COUPON) {
            try {
              ...
              console.warn(
                "Failed to auto-clear server coupon on load:",
                clearErr?.response?.data || clearErr.message || clearErr,
              );
              setAppliedCoupon(null);
              setCouponCode("");
            }
  ...
  const onSelectCouponFromList = (coupon) => {
    ...
    setAppliedCoupon({
      ...local,
      previewDiscount: computeDiscountForCoupon(
        local,
        serverPriceDetails?.totalMRP ?? computeLocalSubtotal(),
      ),
    });
  ...
  const applyCoupon = async () => {
    ...
    const prevServerPrice = serverPriceDetails
      ? { ...serverPriceDetails }
      : null;
    ...
    if (matched) {
      ...
      setAppliedCoupon({
        code: matched.code,
        id: matched._id,
        discountRaw: matched.discount,
        previewDiscount: preview.couponDiscount,
      });
    }
    ...
    try {
      ...
      const endpoints = [
        {
          url: `${API_BASE}/api/buyer/cart/apply-coupon`,
        ...
        {
          url: `${API_BASE}/api/buyer/cart/applyCoupon`,
          body: { couponCode: code },
        },
        { url: `${API_BASE}/api/buyer/cart/apply`, body: { couponCode: code } },
        {
          url: `${API_BASE}/api/buyer/cart/update-coupon`,
          body: { couponCode: code },
        },
      ];
          ...
          if (res?.data?.success) {
            ...
            if (pd) {
              setServerPriceDetails({
                ...
                couponDiscount: Number(
                  pd.couponDiscount ?? pd.discountAmount ?? 0,
                ),
            ...
            setAppliedCoupon((prev) => ({
              ...
              previewDiscount: Number(
                res.data.data?.discountAmount ?? res.data.discountAmount ?? 0,
              ),
            }));
            ...
            if (/invalid|required|not valid/i.test(String(msg))) {
              if (!appliedOnServer && prevServerPrice)
                setServerPriceDetails(prevServerPrice);
              setAppliedCoupon(null);
              setCouponCode("");
      ...
      if (!appliedOnServer) {
        try {
          if (!selectedAddress?.id) {
            ...
            Alert.alert(
              "Coupon Error",
              "Select an address to validate coupon.",
            );
            setApplyingCoupon(false);
            return;
          }
          ...
          if (resOrder?.data?.success) {
            const pd =
              resOrder.data.data?.priceDetails ??
              resOrder.data.data ??
              resOrder.data;
            if (pd) {
              setServerPriceDetails({
            ...
            setAppliedCoupon({
              ...
              serverApplied: true,
              previewDiscount: Number(
                resOrder.data.data?.discountAmount ??
                  resOrder.data.totalDiscount ??
                  0,
              ),
            });
            ...
            const message =
              resOrder?.data?.message || "Coupon could not be validated";
            if (!appliedOnServer && prevServerPrice)
              setServerPriceDetails(prevServerPrice);
            setAppliedCoupon(null);
            setCouponCode("");
          ...
          if (!appliedOnServer && prevServerPrice)
            setServerPriceDetails(prevServerPrice);
          const serverMsg =
            errOrders?.response?.data?.message ||
            errOrders?.message ||
    ...
    if (prev) {
      try {
        ...
        const res = await axios.delete(
          `${API_BASE}/api/buyer/cart/remove-coupon`,
          {
            headers: { Authorization: `Bearer ${token}` },
            timeout: 10000,
          },
  ...
  const updateQuantityInAPI = async (cartItemId, newQuantity) => {
    ...
    try {
      ...
      if (response?.data && response.data.success) {
        const pd =
          response.data.data?.priceDetails ??
        ...
        if (pd) {
          setServerPriceDetails({
            totalMRP: Number(
              pd.totalMRP ?? pd.subtotal ?? computeLocalSubtotal(),
            ),
        ...
        setProducts((prev) =>
          prev.map((p) =>
            p.id === cartItemId ? { ...p, quantity: prevQuantity } : p,
          ),
        );
  ...
  const handleAddressSelect = async (address) => {
    ...
    try {
      ...
      const res = await axios.put(
        `${API_BASE}/api/buyer/addresses/${address._id}/default`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
          timeout: 15000,
      ...
      await fetchBuyerAddresses(); // refresh state
    } catch (err) {
      console.error(
        "Address update failed:",
        err?.response?.data || err.message,
      );
      setSelectedAddress(prev); // rollback
      Alert.alert(
        "Address Error",
  ...
  const deleteAddress = async (address) => {
    ...
    try {
      const token = await AsyncStorage.getItem("userToken");
      const res = await axios.delete(
        `${API_BASE}/api/buyer/addresses/${address.id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const computeTotals = () => {
    ...
    if (
      serverPriceDetails &&
      serverPriceDetails.totalAmount !== undefined &&
      serverPriceDetails.totalAmount !== null
    ) {
      ...
      const serverTotal = Number(
        serverPriceDetails.totalAmount ?? serverSubtotal + serverDelivery,
      );
      const couponDiscount = appliedCoupon
        ? Number(appliedCoupon.previewDiscount || 0)
        : Number(serverPriceDetails.couponDiscount || 0);
    ...
    if (
      appliedCoupon &&
      (appliedCoupon.previewDiscount || appliedCoupon.previewDiscount === 0)
    ) {
      const deliveryChargeFallback = 0;
      const couponDiscount = Number(appliedCoupon.previewDiscount || 0);
      const total = Math.max(
        0,
        subtotal - couponDiscount + deliveryChargeFallback,
      );
      return {
  ...
  if (products.length === 0)
    return (
      <View style={styles.emptyContainer}>
        ...
        >
          <Text allowFontScaling={false} style={styles.shopButtonText}>
            Continue Shopping
          </Text>
        </TouchableOpacity>
      </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {products.length > 0 && (
          <Text allowFontScaling={false} style={styles.deliveryDate}>
            {products[0].deliveryDate || "Delivered soon"}
          </Text>
        )}
        ...
        {/* Donation */}
        <Text allowFontScaling={false}>We don't take any commission</Text>
        <View style={styles.donationContainer}>
          ...
          >
            <Text
              ...
              style={{ fontSize: normalizeFont(13), color: "#333", flex: 1 }}
            >
        ...
        <View style={styles.couponContainer}>
          <View style={styles.couponSection}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text allowFontScaling={false} style={styles.couponSubtitle}>
          ...
          <View style={styles.couponInputContainer}>
            <TextInput
              ...
              onChangeText={(t) => {
                ...
                if (matched) {
                  const previewDiscount = computeDiscountForCoupon(
                    matched,
                    serverPriceDetails?.totalMRP ?? computeLocalSubtotal(),
                  );
                  setAppliedCoupon({
            ...
            {!appliedCoupon ? (
              ...
              >
                {applyingCoupon ? (
                  ...
                  <Text style={{ color: "#fff", fontSize: normalizeFont(12) }}>
                    Apply
                  </Text>
                )}
              </TouchableOpacity>
          ...
          {appliedCoupon && (
            <View style={styles.appliedCouponRow}>
              <Text allowFontScaling={false} style={styles.appliedCouponText}>
                ...
                {appliedCoupon.previewDiscount
                  ? `- ₹${Number(appliedCoupon.previewDiscount).toFixed(2)}`
                  : ""}
              </Text>
            </View>
          )}
                ...
                {couponFetching ? (
                  ...
                  <FlatList
                    ...
                    renderItem={({ item }) => {
                      const subtotal =
                        serverPriceDetails?.totalMRP ?? computeLocalSubtotal();
                      const discountValue = computeDiscountForCoupon(
                        item,
                        subtotal,
                      );
                      return (
                        <TouchableOpacity
                          ...
                          onPress={() => onSelectCouponFromList(item)}
                        >
                          <View style={{ flex: 1 }}>
                            <Text
                              allowFontScaling={false}
                            ...
                            <Text
                              ...
                              style={{ color: "#666" }}
                            >
                              {item.appliesTo?.join?.(", ") || "All Products"}
                            </Text>
                            <Text
                              allowFontScaling={false}
        ...
        <View style={styles.priceSection}>
          <Text allowFontScaling={false} style={styles.priceTitle}>
            Price Details
          </Text>
          <View style={styles.priceRow}>
            ...
            <Text allowFontScaling={false} style={styles.priceValue}>
              ₹{totalsForRender.subtotal.toFixed(2)}
            </Text>
          </View>
          <View style={styles.priceRow}>
            <Text allowFontScaling={false} style={styles.priceLabel}>
            ...
            <Text allowFontScaling={false} style={styles.priceValue}>
              ₹{(-totalsForRender.couponDiscount).toFixed(2)}
            </Text>
          </View>
          <View style={styles.priceRow}>
            <Text allowFontScaling={false} style={styles.priceLabel}>
            ...
            <Text allowFontScaling={false} style={styles.priceValue}>
              ₹{totalsForRender.deliveryCharge.toFixed(2)}
            </Text>
          </View>
          <View style={styles.totalRow}>
            ...
            <Text allowFontScaling={false} style={styles.totalValue}>
              ₹{totalsForRender.totalAmount.toFixed(2)}
            </Text>
          </View>
          {totalsForRender.usedServer && (
            <Text
              allowFontScaling={false}
              style={{
            ...
            >
              Using server-calculated charges
            </Text>
          )}
        </View>

        <View style={styles.commentsSection}>
          <Text allowFontScaling={false} style={styles.commentsTitle}>
            Comments
          </Text>
          <TextInput
      ...
      <View style={styles.bottomPaymentCard}>
        <View style={styles.paymentLeft}>
          <Text allowFontScaling={false} style={styles.totalPrice}>
            ₹{totalsForRender.totalAmount.toFixed(2)}
          </Text>
        </View>
        <TouchableOpacity
        ...
        >
          ...
          <Text allowFontScaling={false} style={styles.proceedButtonText}>
            Proceed to Payment
          </Text>
        </TouchableOpacity>
      </View>
