...
export default function PaymentScreen({ route, navigation }) {
  ...
  const { addressId, deliveryType, comments, paymentMethod } =
    route?.params || {};
    ...
    const createOrderAndGetQR = async () => {
      try {
        ...
        if (mounted && pInfo) {
          ...
          Alert.alert(
            "Warning",
            "Order created but payment details not returned",
          );
        }
      } catch (error) {
        console.log(
  ...
  const handleClosePage = () => {
    Alert.alert("Close Payment Screen?", "Are you sure? Your order is saved.", [
      ...
      >
        ...
        {qrCode ? (
          ...
          >
            <Text
              allowFontScaling={false}
              style={{ color: "#6c757d", fontSize: normalizeFont(12) }}
            >
