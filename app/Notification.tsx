...
export default function NotificationsScreen() {
  ...
  const mapServerNotificationToItem = (n, index) => {
    ...
    return {
      ...
      ctaText: isOrderNotification
        ? "View Order"
        : isProductNotification
          ? "View Product"
          : null,
      ctaType: isOrderNotification
        ? "order"
        : isProductNotification
          ? "product"
          : null,
  ...
  const renderEmpty = () => (
    <View style={styles.empty}>
      <Text
        style={{ color: "#999", fontSize: normalizeFont(14 + 2) }}
        allowFontScaling={false}
      >
    ...
    <SafeAreaView style={styles.safe}>
      ...
      {loading && notifications.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <NotificationCard item={item} onPressCta={handleCta} />
          )}
