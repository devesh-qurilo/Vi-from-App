...
export default function PickupOnlinePayment() {
  ...
  async function createPickupOrderDebug(payload) {
    console.log(
      "→ [DEBUG] POST /api/buyer/pickuporder payload:",
    ...
    try {
      ...
      const res = await axios.post(
        `${BASE_URL}/api/buyer/pickuporder`,
        payload,
        {
          headers: {
      ...
      console.log(
        "← [DEBUG] /api/buyer/pickuporder response:",
        res.status,
        res.data,
      );
      return res;
  ...
  const isPaymentSuccessful = (resData) => {
    ...
    try {
      ...
      if (Array.isArray(resData.payments)) {
        for (const p of resData.payments)
          if (isPaymentSuccessful(p)) return true;
      }
    ...
    const createPickup = async () => {
      ...
      const payload = {
        ...
        paymentMethod:
          (orderPayload.paymentMethod || "UPI").toUpperCase() === "ONLINE"
            ? "UPI"
            : (orderPayload.paymentMethod || "UPI").toUpperCase() === "UPI"
              ? "UPI"
      ...
      try {
        ...
        if (!data.success) {
          Alert.alert(
            "Order Failed",
            data.message || "Could not create pickup order.",
          );
          setLoading(false);
          return;
        }
        ...
        const info = {
          ...
          orderIds:
            data.orderIds || (data.orderId ? [data.orderId] : undefined),
          raw: data,
        ...
        const scan = (obj) => {
          ...
          for (const k of Object.keys(obj)) {
            ...
            if (
              typeof v === "string" &&
              (v.startsWith("http") || v.includes("/api/"))
            ) {
              const candidate = v.startsWith("http")
                ? v
                : `${BASE_URL}${v.startsWith("/") ? "" : "/"}${v}`;
  ...
  useEffect(() => {
    ...
    const orderId = paymentInfo.orderIds
      ? paymentInfo.orderIds[0]
      : paymentInfo._id || paymentInfo.orderId;
    ...
    const poll = async () => {
      ...
      try {
        ...
        const found = await tryEndpointsForStatus(
          orderId || paymentId,
          token,
          extra,
        );
        ...
        const maybeStatus =
          data && (data.status || data.paymentStatus || data.message);
        if (
          maybeStatus &&
  ...
  if (!paymentInfo) {
    return (
      <View style={styles.center}>
        <Text allowFontScaling={false} style={{ color: "#d32f2f" }}>
          No payment info. Please try again.
        </Text>
        <TouchableOpacity
          style={styles.button}
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.amountBox}>
          <Text
            ...
            style={{ color: "#6c757d", marginBottom: moderateScale(6) }}
          >
            Total Amount
          </Text>
        ...
        <View style={styles.upiBox}>
          ...
          <Text allowFontScaling={false} style={styles.upiId}>
            {upiId || "-"}
          </Text>
        ...
        <TouchableOpacity
          style={[
            styles.btn,
            { marginTop: moderateScale(18), opacity: paid ? 0.6 : 1 },
          ]}
        ...
        <TouchableOpacity
          style={[styles.btn, { marginTop: moderateScale(12) }]}
          onPress={goToDonate}
        >
          <Text allowFontScaling={false} style={styles.btnText}>
