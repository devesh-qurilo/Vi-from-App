...
const normalizeOrder = (raw = {}) => {
  const itemsRaw =
    raw.items ?? raw.orderItems ?? raw.products ?? raw.items ?? [];
  ...
  const items = (Array.isArray(itemsRaw) ? itemsRaw : []).map((it) => {
    ...
    const productName =
      productObj.name || it.name || it.productName || "Unknown product";
    ...
    const vendorId =
      (vendorData && (vendorData._id || vendorData.id)) ||
      (raw.vendor && (raw.vendor._id || raw.vendor.id)) ||
      null;
    const vendorName = vendorData
      ? vendorData.name || vendorData.vendorName || ""
      : (raw.vendor?.name ?? "");
    const ratingFromApi =
      it.rating ??
      it.reviewRating ??
      productObj.rating ??
      productObj.avgRating ??
      0;
    const numericRating =
      typeof ratingFromApi === "string"
        ? parseFloat(ratingFromApi)
        : ratingFromApi || 0;
    const canReview = [
      "paid",
      "Paid",
      ...
      "in-process",
      "In-process",
    ].includes(raw.orderStatus ?? raw.status ?? "");
  ...
  const vendorsArray = Array.isArray(raw.vendors)
    ? raw.vendors
    : raw.vendors
      ? [raw.vendors]
      : [];
  ...
  const deliveryFromOrder = Number(
    raw.deliveryCharge ?? raw.shippingCharge ?? raw.delivery ?? 0,
  );
  const vendorDeliverySum = vendorsArray.length
    ? vendorsArray.reduce(
        (s, v) => s + Number(v.deliveryCharge ?? v.delivery ?? 0),
        0,
      )
    : 0;
  ...
  const topTotal = Number(
    raw.totalPrice ??
      ...
      raw.grandTotal ??
      raw.amount ??
      topSubtotal - coupon + delivery,
  );

  return {
    id: raw._id ?? raw.id ?? raw.orderId ?? null,
    orderNumber:
      raw.orderId ?? raw.orderNumber ?? `#${String(raw._id ?? "").slice(0, 6)}`,
    deliveryBy:
      raw.deliveryDate ??
      raw.shippingDate ??
      raw.createdAt ??
...
const ViewOrderDetails = () => {
  ...
  const [order, setOrder] = useState(
    paramOrder ? normalizeOrder(paramOrder) : null,
  );
    ...
    try {
      ...
      if (!resp) {
        ...
        for (let i = 0; i < listUrls.length; i++) {
          try {
            listResp = await axios.get(listUrls[i], {
              headers,
              timeout: 10000,
            });
    ...
    try {
      ...
      if (Array.isArray(uploadedImages) && uploadedImages.length > 0) {
        uploadedImages.forEach((img) => {
          const uri =
            img.uri || img.uriString || (img.assets && img.assets[0]?.uri);
          if (!uri) return;
      ...
      Alert.alert(
        "Error",
        serverMsg || "Failed to submit review. Please try again.",
      );
    } finally {
      setSubmitLoading(false);
    }
      ...
      <SafeAreaView style={styles.safe}>
        <View style={styles.header}>
          <TouchableOpacity style={styles.back} onPress={goBack}>
            <Ionicons
              name="arrow-back"
              size={Math.round(moderateScale(20))}
              color="#333"
            />
          </TouchableOpacity>
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.center}>
          <Ionicons
            ...
            size={Math.round(moderateScale(48))}
            color="#F44336"
          />
          <Text
            allowFontScaling={false}
    ...
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.center}>
          <Text
            allowFontScaling={false}
            style={{ color: "#666", fontSize: normalizeFont(12) }}
          >
            Order not found
          </Text>
        </View>
  ...
  const renderVendorsBlock = () => {
    if (!Array.isArray(vendors) || vendors.length === 0) {
      ...
      return (
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          <Image
            source={{ uri: itemVendor.profilePicture || placeholderImage }}
          ...
          <View style={{ marginLeft: moderateScale(12), flex: 1 }}>
            ...
            <Text allowFontScaling={false} style={styles.detailText}>
              Location:{" "}
              {v.address?.locality || v.address?.city || v.address || "N/A"}
            </Text>
            <Text allowFontScaling={false} style={styles.detailText}>
              Phone: {v.phone || "N/A"}
            </Text>
    ...
    return vendors.map((v, idx) => (
      <View key={v.id || idx} style={{ marginBottom: moderateScale(8) }}>
        <View style={{ flexDirection: "row", alignItems: "flex-start" }}>
          <Image
            source={{ uri: v.avatar || placeholderImage }}
            style={styles.vendorAvatar}
          />
          <View style={{ marginLeft: moderateScale(12), flex: 1 }}>
            <Text allowFontScaling={false} style={styles.detailText}>
            ...
            <Text allowFontScaling={false} style={styles.detailText}>
              Location: {v.address?.locality || v.address?.city || ""}
            </Text>
            <Text allowFontScaling={false} style={styles.detailText}>
              Phone: {v.phone || "N/A"}
            </Text>
            <Text allowFontScaling={false} style={styles.detailSubText}>
              Delivery charge: {formatCurrency(v.deliveryCharge || 0)}
            </Text>
          </View>
        </View>
      </View>
  ...
  const computedTotal =
    summary?.total ?? computedSubtotal - computedCoupon + computedDelivery;
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={{ marginBottom: moderateScale(10) }}>
          ...
          >
            <Text
              allowFontScaling={false}
              style={[styles.detailName, { fontSize: normalizeFont(13) }]}
            >
              {orderStatus}
            </Text>
            <Text
        ...
        <View style={styles.topRow}>
          <View style={{ flex: 1 }}>
            <Text allowFontScaling={false} style={styles.deliveryLabel}>
              Delivery by
            </Text>
            <Text allowFontScaling={false} style={styles.deliveryDate}>
              {formatDateSafe(deliveryBy)}
            </Text>
          </View>
        </View>
            ...
            {items.length === 0 ? (
              ...
              items.map((it, index) => (
                <View key={String(it.id)}>
                  ...
                  >
                    <Image
                      source={{ uri: it.productImage || placeholderImage }}
                    ...
                    <Ionicons
                      ...
                      size={Math.round(moderateScale(18))}
                      color="#ccc"
                    />
                  </TouchableOpacity>
        ...
        <View style={{ marginTop: moderateScale(14) }}>
          <DetailCard title="Vendor Details">{renderVendorsBlock()}</DetailCard>
        </View>
        ...
        >
          <DetailCard title="Order Summary">
            ...
            <View style={styles.summaryRow}>
              ...
              <Text allowFontScaling={false} style={styles.summaryValue}>
                {formatCurrency(computedDelivery)}
              </Text>
            </View>
            ...
            <View style={[styles.summaryRow, { marginTop: moderateScale(8) }]}>
              <Text
                ...
                style={[
                  styles.summaryLabel,
                  { fontWeight: "700", fontSize: normalizeFont(12) },
                ]}
              >
                Total Amount
              </Text>
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            >
              ...
              <Text allowFontScaling={false} style={styles.modalImageText}>
              ...
              >
                <View style={styles.modalImageUploadContent}>
                  <Ionicons
                    name="camera-outline"
                    size={Math.round(moderateScale(28))}
                    color="#999"
                  />
                  <Text
                    allowFontScaling={false}
                    style={styles.modalImageUploadText}
                  >
              ...
              <Text allowFontScaling={false} style={styles.modalReviewText}>
                Write a review
              </Text>
              <TextInput
                allowFontScaling={false}
