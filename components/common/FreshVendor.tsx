...
const FreshVendor = () => {
  ...
  const fetchVendors = async () => {
    try {
      ...
      const normalized = (Array.isArray(fetched) ? fetched : []).map((v) => {
        const id =
          v._id || v.id || v.vendorId || (v.raw && (v.raw._id || v.raw.id));
        const name =
          v.name || v.vendorName || v?.profile?.name || "Unknown Vendor";
        const image =
          v.profilePicture ||
          v.image ||
          v.profileImage ||
        ...
        const distance =
          v.distance ||
          v.distanceFromVendor ||
          (typeof v.distanceValue === "number"
            ? `${v.distanceValue} km`
            : "0.0 km");
        ...
        return {
          ...
          rating,
          distance,
          category: normalizeCategoryArray(
            v.categories || v.category || v.categoriesList,
          ),
  ...
  const openVendorDetails = (vendor) => {
    if (!vendor) return;
    const vendorId =
      vendor?.id || vendor?._id || vendor?.raw?._id || vendor?.raw?.id;
    if (!vendorId) return;
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={vendors}
        keyExtractor={(v, idx) =>
          v.id || v._id || v.raw?.id || v.name || String(idx)
        }
        horizontal
