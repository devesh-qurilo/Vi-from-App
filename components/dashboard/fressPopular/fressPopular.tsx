...
const cardStyles = StyleSheet.create({
  ...
  card: { borderRadius: moderateScale(11), overflow: "hidden" },
  image: {
    width: "100%",
  ...
  const fetchFreshPopular = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE}/api/buyer/fresh-and-popular`,
        {
          headers,
          timeout: 10000,
        },
  ...
  const openProductDetails = (product) => {
    try {
      if (!product || !product._id) {
        console.warn(
          "openProductDetails: missing product or product ID",
          product,
        );
        return;
      }
  ...
  return (
    <View>
      <View style={styles.headerRow}>
        ...
        <TouchableOpacity
          style={styles.seeButton}
          onPress={() => navigation.navigate("ViewAllFressPop")}
        >
          <Text allowFontScaling={false} style={styles.link}>
            See All
          </Text>
      ...
      {data.length > 0 ? (
        <FlatList
          ...
          keyExtractor={(item, index) =>
            (item._id || item.id || String(index)).toString()
          }
          horizontal
          showsHorizontalScrollIndicator={false}
          contentContainerStyle={{ paddingHorizontal: moderateScale(10) }}
          renderItem={({ item }) => (
            <ProductCard item={item} onPress={openProductDetails} />
          )}
        />
      ) : (
