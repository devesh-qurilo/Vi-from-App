...
const MyCart = () => {
  ...
  const selectVendor = async (vendorId) => {
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        console.warn(
          "Failed to select vendor on server:",
    ...
    async (tokenArg) => {
      ...
      try {
        ...
        if (res.ok && json?.success) {
          ...
          items.forEach((item) => {
            ...
            if (!vendorsMap.has(vendorId)) {
              vendorsMap.set(vendorId, {
                ...
                pickupLocationText:
                  `${vendor.address?.houseNumber || ""} ${vendor.address?.street || ""}, ${vendor.address?.locality || ""}, ${vendor.address?.city || ""}`.trim(),
                address: vendor.address || {},
                about: vendor.about || "",
                latitude:
                  vendor.address?.latitude || vendor.geoLocation?.[1] || null,
                longitude:
                  vendor.address?.longitude || vendor.geoLocation?.[0] || null,
                items: [],
                priceDetails: {},
                selected: false,
              });
            ...
            const subtotal = vendorData.items.reduce(
            ...
            vendorData.priceDetails = {
              ...
              couponDiscount: Number(
                vendorPriceDetails.discount ||
                  vendorPriceDetails.couponDiscount ||
                  0,
              ),
              deliveryCharge: Number(vendorPriceDetails.deliveryCharge || 0),
              totalAmount: Number(
                vendorPriceDetails.totalAmount ||
                  Math.max(
                    0,
                    subtotal -
                      (vendorPriceDetails.discount ||
                        vendorPriceDetails.couponDiscount ||
                        0) +
                      (vendorPriceDetails.deliveryCharge || 0),
                  ),
              ),
            };
          });
          ...
          if (persisted) {
            toSelect = vendorsArray.find(
              (v) => String(v.id) === String(persisted),
            );
          }
          ...
          if (!toSelect) {
            const selectedVendorIds =
              json.data && Array.isArray(json.data.selectedVendors)
                ? json.data.selectedVendors
                : [];
            if (selectedVendorIds.length > 0) {
              toSelect = vendorsArray.find(
                (v) => v.id === selectedVendorIds[0],
              );
            }
          }
  ...
  const fetchHighlightedCoupons = async (token) => {
    try {
      ...
      const res = await fetch(`${BASE_URL}${GET_HIGHLIGHTED_COUPONS}`, {
        headers,
      });
      const json = await res.json().catch(() => ({}));
  ...
  const removeItem = async (itemId) => {
    ...
    try {
      const res = await fetch(
        `${BASE_URL}${DELETE_CART_ITEM_ENDPOINT}${itemId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
    ...
    try {
      ...
      if (res.ok && json.success) {
        ...
        const newPriceDetails = {
          totalMRP: Number(
            pd.totalMRP ?? pd.totalAmount ?? priceDetails.totalMRP ?? 0,
          ),
          couponDiscount: Number(pd.discount ?? pd.couponDiscount ?? 0),
          deliveryCharge: Number(pd.deliveryCharge ?? 0),
          totalAmount: Number(
            pd.totalAmount ??
              Number(pd.totalMRP ?? 0) -
                Number(pd.discount ?? pd.couponDiscount ?? 0) +
                Number(pd.deliveryCharge ?? 0) ??
              0,
          ),
        };
        ...
        const updatedSelectedVendor =
          updatedVendors.find((v) => v.id === selectedVendor.id) ||
          selectedVendor;
    ...
    const getDeleveryCharges = async () => {
      try {
        ...
        const res = await axios.get(
          `${BASE_URL}/api/buyer/cart/delivery-charge`,
          {
            headers: {
  ...
  const subtotal = selectedVendorItems.reduce(
  ...
  const serverHas =
    priceDetails &&
    priceDetails.totalAmount !== undefined &&
    priceDetails.totalAmount !== null;
  const baseSubtotal = serverHas
    ? Number(priceDetails.totalMRP ?? subtotal)
    : subtotal;
  const baseDelivery = serverHas
    ? Number(priceDetails.deliveryCharge ?? 0)
    : Number(priceDetails.deliveryCharge ?? 0);
  const serverTotal = serverHas
    ? Number(priceDetails.totalAmount)
    : Number(
        baseSubtotal - Number(priceDetails.couponDiscount || 0) + baseDelivery,
      );

  const finalAmount = Math.max(
    0,
    Number((serverTotal - effectiveCouponDiscount).toFixed(2)),
  );
  ...
  const applyQuantityChange = () => {
    ...
    if (!editingItemId || newQty <= 0) {
      closeQtyModal();
      return;
    }

    try {
      // ✅ MyCart ka real updater
      updateQuantity(editingItemId, newQty);
    } catch (err) {
    ...
    <View style={styles.cartCard}>
      ...
      <View style={styles.productDetails}>
        <View style={styles.productInfo}>
          <Text allowFontScaling={false} style={styles.productTitle}>
            {item.title}
          </Text>
          <View style={styles.priceContainer}>
            <Text allowFontScaling={false} style={styles.priceText}>
              <Text style={{ color: "rgba(66, 66, 66, 1)", fontWeight: "500" }}>
                MRP{" "}
              </Text>
        ...
        >
          <Image
            source={require("../../assets/via-farm-img/icons/deleteBtn.png")}
            style={{ width: scale(15), height: scale(18), tintColor: "grey" }}
          />
        </TouchableOpacity>
        <View style={styles.quantityContainer}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Text style={styles.quantityButtonText} allowFontScaling={false}>
              +
            </Text>
          </TouchableOpacity>
        </View>
    ...
    return (
      ...
      >
        ...
        >
          <Image
            source={{
              uri: vendor.profilePicture || "https://via.placeholder.com/40",
            }}
          ...
          <View style={styles.vendorInfoHeader}>
            ...
            <Text allowFontScaling={false} style={styles.vendorItemCount}>
  ...
  const EmptyCart = () => (
    <View style={styles.emptyCartContainer}>
      ...
      />
      <Text allowFontScaling={false} style={styles.emptyCartTitle}>
        Your Cart is Empty
      </Text>
      <Text allowFontScaling={false} style={styles.emptyCartSubtitle}>
      ...
      >
        <Text allowFontScaling={false} style={styles.shopNowText}>
          Shop Now
        </Text>
      </TouchableOpacity>
    </View>
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
      ...
      <View style={styles.header}>
        ...
        <Text allowFontScaling={false} style={styles.headerTitle}>
          My Cart {loading && vendors.length > 0 && "(Updating...)"}
        </Text>
        <View />
      </View>
        ...
        >
            ...
            <View style={styles.couponSection}>
              ...
              >
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  <Image
                    source={require("../../assets/via-farm-img/icons/promo-code.png")}
                  />
                  <View style={{ marginLeft: moderateScale(8) }}>
                    ...
                    >
                      Tap a coupon to apply quickly
                    </Text>
                  </View>
                </View>

                <TouchableOpacity
                  onPress={() => setShowCouponsDropdown((prev) => !prev)}
                >
                  <Text
                    allowFontScaling={false}
                    style={{ fontSize: normalizeFont(10), color: "#0077ff" }}
                  >
                    {showCouponsDropdown ? "Hide" : "View Coupons"}
                  </Text>
                </TouchableOpacity>
              </View>
              ...
              <Modal
                visible={showCouponsDropdown}
                transparent
                animationType="fade"
                onRequestClose={() => setShowCouponsDropdown(false)}
              >
                <View style={styles.centeredOverlay}>
                  <Pressable
                    style={styles.modalBackground}
                    onPress={() => setShowCouponsDropdown(false)}
                  />
                      ...
                      >
                        {fetchingCoupons ? (
                          ...
                          highlightedCoupons.map((c) => {
                            const isPercentage =
                              (c.discount?.type || "").toLowerCase() ===
                              "percentage";
                            const label = isPercentage
                              ? `${c.discount.value}% off`
                              : `₹${c.discount.value} off`;
                            const subtotalLocal =
                              priceDetails?.totalMRP ?? subtotal;
                            const previewDiscount = isPercentage
                              ? Number(
                                  (
                                    (subtotalLocal *
                                      Number(c.discount.value || 0)) /
                                    100
                                  ).toFixed(2),
                                )
                              : Number(
                                  Number(c.discount.value || 0).toFixed(2),
                                );
                            return (
                              <TouchableOpacity
                                key={c._id}
              ...
              <View style={styles.couponInputContainer}>
                ...
                />
                {appliedCoupon &&
                appliedCoupon.vendorId === selectedVendor?.id ? (
                  <TouchableOpacity
              ...
              {couponError ? (
                <Text allowFontScaling={false} style={styles.couponError}>
                  {couponError}
                </Text>
              ) : appliedCoupon &&
                appliedCoupon.vendorId === selectedVendor?.id ? (
                <Text allowFontScaling={false} style={styles.couponSuccess}>
                  Coupon applied! ₹{Number(effectiveCouponDiscount).toFixed(2)}{" "}
                  discount
                </Text>
              ) : null}
            </View>
          ...
          {selectedVendor && (
            <View style={styles.priceSection}>
              <Text allowFontScaling={false} style={styles.priceSectionTitle}>
                Price Details for {selectedVendor?.name}
              </Text>

              <View style={styles.priceRow}>
                <Text allowFontScaling={false} style={styles.priceLabel}>
                  Total MRP
                </Text>
                <Text allowFontScaling={false} style={styles.priceValue}>
              ...
              <View style={styles.priceRow}>
                ...
                <Text allowFontScaling={false} style={styles.discountValue}>
                  -₹{Number(effectiveCouponDiscount).toFixed(2)}
                </Text>
              </View>
              ...
              <View style={{ marginVertical: normalizeFont(10) }}>
                ...
                >
                  ...
                  <Text
                    allowFontScaling={false}
                    style={{ fontSize: normalizeFont(10), fontWeight: 500 }}
                  >
                    ₹{delivryCharge?.data?.deliveryCharge}
                  </Text>
                </View>
                <View
                  style={{
      ...
      {selectedVendor && selectedVendorItems.length > 0 && (
        <View style={styles.checkoutContainer}>
          <TouchableOpacity style={styles.checkoutButton} onPress={openModal}>
            <Image
              source={require("../../assets/via-farm-img/icons/UpArrow.png")}
            />
            <Text allowFontScaling={false} style={styles.checkoutText}>
              Place Order for {selectedVendor?.name}
            </Text>
          </TouchableOpacity>
        </View>
      )}
              ...
              <View style={styles.modalContainer}>
                ...
                <View style={styles.locationInfo}>
                  ...
                  <View style={styles.locationDetails}>
                    ...
                    >
                      {selectedVendorDetails?.pickupLocationText ||
                        "Loading location..."}
                    </Text>
                    <Text
                      allowFontScaling={false}
                    ...
                    >
                      {selectedVendorDetails?.address?.locality ||
                        "Location details"}
                    </Text>
                  </View>
                ...
                <View style={styles.slotSection}>
                  <Text allowFontScaling={false} style={styles.slotTitle}>
                    Pick a slot
                  </Text>
                  ...
                  <View style={styles.dateRow}>
                    ...
                    >
                      ...
                      <Text allowFontScaling={false} style={styles.dateIcon}>
                        📅
                      </Text>
                    </TouchableOpacity>
                  </View>
                  ...
                  <View style={styles.timeRow}>
                    ...
                    <View style={styles.timeContainer}>
                      ...
                      >
                        <Text allowFontScaling={false} style={styles.ampmText}>
                          {" "}
                          {startAMPM}
                        </Text>
                      </TouchableOpacity>
                      <Text allowFontScaling={false} style={styles.timeTo}>
                        to
                      </Text>
                      <TouchableOpacity
                        style={styles.timeInput}
                ...
                >
                  ...
                  <TouchableOpacity
                    ...
                    style={{
                      ...
                      borderWidth: 1,
                      borderColor:
                        paymentMethod === "cash" ? "#FFA500" : "#ddd",
                      borderRadius: moderateScale(8),
                      marginBottom: moderateScale(10),
                      backgroundColor:
                        paymentMethod === "cash" ? "#FFF8E1" : "#fff",
                    }}
                  >
                  ...
                  <TouchableOpacity
                    ...
                    style={{
                      ...
                      borderWidth: 1,
                      borderColor:
                        paymentMethod === "online" ? "#FFA500" : "#ddd",
                      borderRadius: 8,
                      backgroundColor:
                        paymentMethod === "online" ? "#FFF8E1" : "#fff",
                    }}
                  >
                    {/* ✅ TEXT FIX */}
                    <Text
                    ...
                    <View
                      style={{
                        ...
                        borderWidth: 2,
                        borderColor:
                          paymentMethod === "online" ? "#FFA500" : "#ccc",
                        alignItems: "center",
                        justifyContent: "center",
                        flexShrink: 0,
                      }}
                ...
                <Text allowFontScaling={false} style={styles.vendorTitle}>
                  Vendor Details
                </Text>
                ...
                <View style={styles.vendorInfo}>
                  <Image
                    ...
                    source={{
                      uri:
                        selectedVendorDetails?.profilePicture ||
                  ...
                  <View style={styles.vendorDetails}>
                    <Text allowFontScaling={false} style={styles.vendorName}>
                      {selectedVendorDetails?.name
                        ? selectedVendorDetails.name.charAt(0).toUpperCase() +
                          selectedVendorDetails.name.slice(1)
                        : "Vendor Name"}
                    </Text>
      ...
      <Modal visible={showSuccessModal} transparent animationType="fade">
        <View style={styles.successModalOverlay}>
          <View style={styles.successModalBox}>
            <Image
              source={require("../../assets/via-farm-img/icons/complete.png")}
              style={{
                width: scale(80),
                height: scale(70),
          ...
          >
            ...
            <View style={styles.deliveryModalHeader}>
              <Text allowFontScaling={false} style={styles.modalTitle}>
                Select Delivery Option for {selectedVendor?.name}
              </Text>
            </View>
            ...
            <View style={styles.optionsContainer}>
              ...
              >
                <View style={styles.optionContent}>
                  <Text allowFontScaling={false} style={styles.optionTitle}>
                    Pickup your package from vendor's location
                  </Text>
              ...
              >
                <View style={styles.optionContent}>
                  <Text allowFontScaling={false} style={styles.optionTitle}>
                    Get your package delivered to your doorstep
                  </Text>
                  <View style={styles.optionSubtitle}>
                    <Text allowFontScaling={false} style={styles.text}>
                      Delivery
