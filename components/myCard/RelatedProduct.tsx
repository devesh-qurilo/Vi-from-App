...
const RelatedProduct = ({ productId: propProductId, cardWidth }) => {
  ...
  const getToken = async () => {
    ...
    if (!token) {
      for (const key of POSSIBLE_TOKEN_KEYS) {
        try {
          ...
          console.warn(`Error getting token from ${key}:`, e);
        }
      }
    }
    if (token && typeof token === "string" && token.startsWith("{")) {
      try {
        ...
        token =
          parsed?.accessToken || parsed?.token || parsed?.data?.token || token;
      } catch (e) {
  ...
  const fetchSimilar = async (mounted = true) => {
    try {
      ...
      setError(
        err?.response?.data?.message || "Failed to load related products",
      );
  ...
  const handleQuantityChange = async (delta, product) => {
    try {
      ...
      if (newQuantity < 1) {
        ...
        await removeCartItemFromServer(existing.cartItemId);
        return;
      }
      ...
      const ok = await updateCartItemQuantity(existing.cartItemId, newQuantity);
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const qty = cartItems[String(item.id)]
            ...
            : 0;
          const subtitle =
            (item.vendor && item.vendor.name) || item.vendorName || "";
