...
export default function AllOrders() {
    ...
    try {
      ...
      const formattedOrders = (sourceArray || []).map((o) => {
        ...
        const uniqueUnits =
          Array.from(
            new Set(
              products.map((p) => p.product?.unit || p.unit).filter(Boolean),
            ),
          ).join(", ") || "N/A";
        ...
        const commentsForUI =
          orderTypeVal.toLowerCase() === "pickup"
            ? ""
            : String(rawComments || "");
      ...
      Alert.alert(
        "Error",
        "Failed to fetch orders. Check console for details.",
      );
      setOrders([]);
      setFilteredOrders([]);
    } finally {
  ...
  const applyFiltersAndSearch = () => {
    ...
    if (typeof filters.priceRange === "number" && filters.priceRange < 5000) {
      result = result.filter(
        (order) => Number(order.totalPrice || 0) <= filters.priceRange,
      );
    }
