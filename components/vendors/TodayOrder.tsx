...
export default function OrdersScreen() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (
        res.data &&
        (res.data.success || res.data.data || Array.isArray(res.data))
      ) {
        ...
        const formattedOrders = (source || []).map((o) => {
          ...
          const units =
            Array.from(
              new Set(
                products
                  .map((p) =>
                    p.product && p.product.unit ? p.product.unit : null,
                  )
                  .filter(Boolean),
              ),
            ).join(", ") || "N/A";
          ...
          const deliveredAt = o.pickupSlot
            ? o.pickupSlot.date
              ? `${o.pickupSlot.date} ${o.pickupSlot.startTime ?? ""}`
              : String(o.pickupSlot)
            : o.createdAt
          ...
          return {
            ...
            buyer:
              o.buyer?.name ||
              (typeof o.buyer === "string" ? o.buyer : "Unknown Buyer"),
            contact:
              o.buyer?.mobileNumber ||
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    ...
    const optimistic = (list) =>
      list.map((o) =>
        o.id === orderId ? { ...o, status: newStatus, __updating: true } : o,
      );
    ...
    try {
      ...
      for (let i = 0; i < candidateUrls.length; i++) {
        ...
        try {
          ...
          if (err?.response?.status === 404) {
            console.warn(
              `[StatusUpdate] ${url} returned 404 — trying next candidate`,
            );
            continue;
          } else {
            console.warn(
              `[StatusUpdate] ${url} failed:`,
      ...
      setOrders((prev) =>
        prev.map((o) =>
          o.id === orderId
            ? { ...o, status: serverStatus, __updating: false }
            : o,
        ),
      );
    } catch (err) {
      ...
      if (err?.response) {
        ...
        if (st === 401) {
          ...
          Alert.alert(
            "Not Found",
            "Order endpoint not found (404). Check order id and backend route.",
          );
        } else {
          Alert.alert(
            "Update failed",
            `Server returned ${st}: ${data?.message || JSON.stringify(data)}`,
          );
        }
